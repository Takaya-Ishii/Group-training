...
--INSERT INTO group_table (group_name, summary) VALUES ('25卒内定者グループ', '2025年内定者用グループ 配属決定前まで使用 インフラ・アプリ共有') ON CONFLICT (group_ID) DO NOTHING;
--INSERT INTO group_table (group_name, summary) VALUES ('インフラ研修グループ', 'インフラチーム研修用グループ') ON CONFLICT (group_ID) DO NOTHING;

INSERT INTO member_table (group_ID, username, isTeacher) VALUES (1, 'hogehoge@example.jp', FALSE) ON CONFLICT (group_user_ID) DO NOTHING;
INSERT INTO member_table (group_ID, username, isTeacher) VALUES (1, 'fugafuga@example.jp', TRUE)  ON CONFLICT (group_user_ID) DO NOTHING;
